...
boot_anim = [f"{Style.BRIGHT}{Fore.LIGHTWHITE_EX}Booting {Fore.RED}Boo {Fore.RESET}{Fore.LIGHTWHITE_EX}Bot",
             f"{Fore.RED}25%",
             f"{Fore.YELLOW}50%",
             f"{Fore.LIGHTYELLOW_EX}75%",
             f"{Fore.LIGHTGREEN_EX}99%",
             f"{Fore.LIGHTBLUE_EX}Boo Bot booted"]
if not boot:
...
async def massdm():
        ...
        input(f"\n{Fore.WHITE}Thanks for using {Fore.YELLOW}Boo\nPress Enter to return to the main menu")
        return await main()
...
async def exit():
        await client.close()
        input(f"{Fore.WHITE}Thanks for using {Fore.YELLOW}Boo\nPress Enter 5 times to close the program.")
...
'''))
    print(f'''
{Fore.LIGHTWHITE_EX}                                   Made by {Fore.YELLOW}BooIsLive 
{Fore.LIGHTWHITE_EX}Check out the github page for updates: {Fore.LIGHTBLUE_EX}https://github.com/UnknownSkiid/           
...
def start():
    @client.event
    async def on_ready():
        await client.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.watching, name="github.com/UnknownSkiid"))
        os.system('cls' if os.name == 'nt' else 'clear')
        return await main()
